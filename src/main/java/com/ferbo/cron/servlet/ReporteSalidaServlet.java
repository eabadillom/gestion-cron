...
public class ReporteSalidaServlet extends HttpServlet {

	private static final long serialVersionUID = -3495651249588047684L;
	private static Logger log = LogManager.getLogger(ReporteEntradaServlet.class);
	
	public ReporteSalidaServlet() {
		super();
	}
	...
	throws ServletException, IOException {
		...
		ReporteSalidasJR  salidasJR     = null;
		...
		try {
			...
			log.info("Reporte de salidas {}, Periodo: {} al {}",
					cliente.getNombre(),
			...
			salidasJR = new ReporteSalidasJR(conn, fsTools.getResourcePath(logoPath));
			
			bytes = salidasJR.getPDF(periodoSemanal.getFechaInicio(), periodoSemanal.getFechaFin(), cliente.getIdCliente(), null, null);
			
			response.setContentType("application/pdf");
			response.setHeader("Content-Type", "application/pdf");
			response.setHeader("Content-Disposition", "attachment; filename=\"ReporteSalidas.pdf\"");
	...
	throws ServletException, IOException {
		...
		try {
			...
			numeroCliente = request.getParameter("numeroCliente");
			mailBO = SendMailSalidasBL.getInstance();
			...
			if(numeroCliente == null || "".equals(numeroCliente.trim())) {
				...
				log.info("El reporte de salidas del cliente {} terminó correctamente.", cliente.getNombre());
			}
